...
(define interference
  (class object%
    ...
    (define/public (add-edges write-block live-block)
      (for ([i (in-range (sub1 (pvector-length live-block)))])
        (define w (pvector-ref write-block i))
        (define r (pvector-ref live-block (+ i 1)))
    ...
    (define/public (solve write-block live-block)
      (for ([i (in-range (pvector-length write-block))])
        (define w (pvector-ref write-block i))
        (define r (pvector-ref live-block (+ i 1)))
