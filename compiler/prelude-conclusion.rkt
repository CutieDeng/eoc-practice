...
(define pass-prelude-and-conclusion
  (class object%
    ...
    (define (get-prelude p) (match p [(X86Program info _)
      ...
      (Block (ordl-make-empty symbol-compare) (vector->pvector insts))
    ]))
    ...
    (define/public (pass p) (match p [(X86Program info blocks)
      ...
      (define blocks^
        (for/fold ([a (ordl-make-empty integer-compare)]) ([(tag block) (in-dict blocks)])
          ...
          (define instr*^
            (cond
              [(pvector-empty? instr*) (pvector-cons-right instr* (Jmp 1))]
              [else (define last-instr (pvector-view-right instr*))
                (if (Jmp? last-instr) instr* (pvector-cons-right instr* (Jmp 1)))
              ]
            ))
