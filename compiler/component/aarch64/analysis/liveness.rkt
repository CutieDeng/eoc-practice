...
;; Provides aarch64-specific instruction def/use analysis.
;;
;; ============================================================================
...
(define (make-aarch64-liveness-interface vreg-index cfg)
  ...
  (define pred-map (cfg-predecessors cfg))
...
(define aarch64-liveness-interface%
  (class* object% (liveness-interface<%>)
    (init-field id->idx-map     ; ordered-map[symbol -> int]
                ...
                var-count-val   ; int
                predecessors-map) ; ordered-map[BlockId -> (listof BlockId)]
    ...
    (define/public (block-predecessors cfg bid)
      (list->pvector (dict-ref predecessors-map bid '())))
...
;; Original block-id-compare returns symbols, need numeric version
