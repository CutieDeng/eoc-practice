...
(define (emit-terminator term [config default-config])
  ...
  (match term
    ...
    [(Term:jump target)
     (format "~ab .L~a" indent (BlockId-id target))]

    [(Term:cond cond then-target else-target)
     (string-append
      (format "~ab.~a .L~a\n" indent cond (BlockId-id then-target))
      (format "~ab .L~a" indent (BlockId-id else-target)))]
...
(define (emit-block block [config default-config])
  ...
  (define label-str
    (match label
      ...
      [(Label:id id) (format ".L~a:" id)]
      [_ (format ".L~a:" (BlockId-id (AsmBlock-id block)))]))
