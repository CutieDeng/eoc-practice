...
(define analyzer
  (class object%
    ...
    (define/private (analyze-graph-batch:impl)
      (cond [(pvector-empty? worklist) (void)]
      [else
        (define-values (worklist^ bb-id) (pvector-pop-right worklist))
        ...
        (define bb-input (match pred-ids
          [(pvector) default]
          [_
            (for/fold ([s bottom]) ([p-id (in-pvector pred-ids)])
        ...
        (cond
          ...
          [else
            ...
            (for (
              ...
              (set! worklist (pvector-cons-right worklist succ-id))
            )
          ])
    ...
    (define/private (analyze-graph-batch batch)
      (define-values (c^ w^)
        (for/fold ([c 0] [w (pvector-empty)]) ([i (in-bset batch)] #:do [(define j (fn i))])
          (values (bset-add c j) (pvector-cons-right w j))
        ))
      (set! change-set c^)
